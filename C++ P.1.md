...
#### cout 的新花样
cout 可以打印字符串，也可以打印变量，以下的程序便是打印变量：
```c++
cout << carrots;
```

程序没有打印"carrots"的字符串，而是打印存储在carrots中的数值。
该操作流程为： cout 将 carrots 替换为其当前值25,然后把值转换为合适的输出字符。
cout 可以用于数字和字符串。
***但这里有个重点：***
**整数的25与字符串的"25" 是完全不一样的。**
字符串"25" 存储的是书写该数字时使用的字符，则字符2和5。程序在内部存储的是字符2和字符5的编码。
要打印字符串，cout只需打印字符串中各个字符即可。
但整数25被存储为数值，在计算机里是存储为二进制数。在打印赚钱，cout必须将整数形式的数字转化成字符串形式。
在这方面 cout 很聪明，知道carrots是一个需要转换的整数。

如果用C的话，C就比较的蠢，所以是这样写的：
```c
printf("Printing a string:%s/n", "25");
printf("Printing an integer:%d/n",25);
```

printf()就必须得要用特殊代码（%s和%d）来指出打印的是字符串还是整数。
